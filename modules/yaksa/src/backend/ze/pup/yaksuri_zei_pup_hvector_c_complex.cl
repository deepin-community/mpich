...
__kernel void yaksuri_zei_kernel_pack_PROD_hvector_c_complex(__global const void *inbuf, __global void *outbuf, unsigned long count, __global const yaksuri_zei_md_s *__restrict__ md)
{
    ...
    intptr_t stride1 = md->u.hvector.stride;
    float2 dest;
    float2 src = *((const float2 *) (const void *) (sbuf + x0 * extent + x1 * stride1 + x2 * sizeof(float2)));
...
__kernel void yaksuri_zei_kernel_unpack_PROD_hvector_c_complex(__global const void *inbuf, __global void *outbuf, unsigned long count, __global const yaksuri_zei_md_s *__restrict__ md)
{
    __global const char *__restrict__ sbuf = (__global char *) inbuf;
    __global char *__restrict__ dbuf = (__global char *) outbuf;
    dbuf = dbuf - md->true_lb;
    ...
    float2 temp_dest = *((float2 *) (void *) (dbuf + x0 * extent + x1 * stride1 + x2 * sizeof(float2)));
    dest.x = temp_dest.x * src.x - temp_dest.y * src.y;
    dest.y = temp_dest.x * src.y + temp_dest.y * src.x;
